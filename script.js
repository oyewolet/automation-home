// Fetch temperature from Blynk (V10) and update UI
function fetchBlynkTemperature() {
  fetch("/.netlify/functions/blynk-relay?url=" + encodeURIComponent("https://blynk.cloud/external/api/get?token=wUchuR6qCNFKyUae7vtCj8kozCKa5HVW&V10"))
...
// Blynk global control function
function blynkSetPin(pin, value) {
  fetch(
    "/.netlify/functions/blynk-relay?url=" + encodeURIComponent(`https://blynk.cloud/external/api/update?token=wUchuR6qCNFKyUae7vtCj8kozCKa5HVW&${pin}=${value}`)
  )
    .then((r) => r.json())
    .then(console.log)
    .catch(console.error);
...
function toggleBulb() {
  ...
  toggleDevice("light1");
  blynkSetPin("V1", lamp.src.includes("light_bulb_on.png") ? 1 : 0);
}
...
function togglefan() {
  ...
  status.textContent = isOff ? "ON" : "OFF";
  toggleDevice("fan");
  blynkSetPin("V4", fan.src.endsWith("fan-on.png") ? 1 : 0);
}
...
function toggleBulb1() {
  ...
  status.textContent = isOff ? "ON" : "OFF";
  toggleDevice("light2");
  blynkSetPin("V2", lamp.src.includes("light_bulb_on.png") ? 1 : 0);
}
...
function toggleBulb2() {
  ...
  status.textContent = isOff ? "ON" : "OFF";
  toggleDevice("light3");
  blynkSetPin("V3", lamp.src.includes("light_bulb_on.png") ? 1 : 0);
}
// DOORS
// Device Toggle Functions
function toggleBackDoor() {
  ...
  toggleDevice("door1");
  blynkSetPin("V5", Door.getAttribute("data-state") === "unlocked" ? 0 : 1); // 1=locked, 0=unlocked
}
...
function toggleFrontDoor() {
  ...
  status.textContent = locked ? "OPEN" : "CLOSE";
  toggleDevice("door2");
  blynkSetPin("V6", Door.getAttribute("data-state") === "unlocked" ? 0 : 1); // 1=locked, 0=unlocked
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetchStatus();
  // Optionally, fetch Blynk temperature on page load
  fetchBlynkTemperature();

  // Optionally, add a button for manual Blynk temperature refresh
  const tempText = document.getElementById("temperatureText");
  if (tempText) {
    const btn = document.createElement("button");
    btn.textContent = "Refresh Blynk Temp";
    btn.style.marginLeft = "10px";
    btn.onclick = fetchBlynkTemperature;
    tempText.parentNode.insertBefore(btn, tempText.nextSibling);
  }

  // Automatically update Blynk temperature every 10 seconds
  setInterval(fetchBlynkTemperature, 10000);
});
